...
public class _DownloadHandler : DownloadHandlerScript
{
    ...
    private event Action<long> _eventDownloadSpeed = null; //下载速度
    #endregion
    ...
    {
        //DebugUtils.DebugerExtension.Log(this,"接收到的数据长度:" + ((float)(dataLength / 1024)).ToString("0.0") + "KB/" + ((float)(data.Length / 1024)).ToString("0.0") + "KB");
        if (data == null || data.Length == 0)
        {
            DebugUtils.DebugerExtension.Log(this,"没有获取到数据缓存！");
            return false;
        }
        fs.Write(data, 0, dataLength);
        _downedLength += dataLength;
       // DebugUtils.DebugerExtension.Log(this,"数据进度=>" + ((float)(_downedLength / 1024)).ToString("0.0") + "KB/" + ((float)(_contentLength / 1024)).ToString("0.0") + "KB");
        
        if (_eventProgress != null)
            _eventProgress.Invoke((float)_downedLength * 1.0f / _contentLength);   //通知进度消息
    ...
    {
        //DebugUtils.DebugerExtension.Log(this,"下载完成！");
        ...
        if (File.Exists(TempFilePath))
        {
            ...
            //DebugUtils.DebugerExtension.Log(this,"重命名文件！");
        }
        else
        {
            DebugUtils.DebugerExtension.Log(this,"生成文件失败=>下载的文件不存在！");
        }
    ...
    {
        //DebugUtils.DebugerExtension.Log(this,"下载总量=>" + _downedLength);
        fs.Close();
        fs.Dispose();
    }
    ...
    protected override void ReceiveContentLength(int contentLength)
    {
        ...
        DebugUtils.DebugerExtension.Log(this,string.Format("收到头部信息==>>数据总长度{0}", contentLength));
    }
}
